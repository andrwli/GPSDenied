'''
Example for Navigation with RC override and PID controller
...
def move_to_target(vehicle, target_waypoint, dt=0.5):
    """Moves vehicle to the target coordinate. Must be interrupted if 
    vehicle only needs to move in the direction of target coordinate. 
...
def correct_thrust(vehicle, target_relative_altitude=50):
    ...
    1000 - Min throttle or decend at the fastest rate 
    1500 - 50% throttle or hold current altitude 
    2000 - 100% throttle or climb at the fastest rate 
...
def correct_yaw(vehicle, target_waypoint, pid_controller=PID()):
    """Corrects the yaw so that vehicle heads toward target_waypoint."""
...
def get_calculated_yaw(vehicle, target_waypoint, pid_controller):
    ...
    1000 - counter-clockwise yaw (full left stick)
    1500 - hold current yaw (mid stick)
    2000 - clockwise yaw (full right stick)
